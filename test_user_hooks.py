...
class SafeLightningModule(L.LightningModule):
    ...
    def on_weavelet_train_epoch_start(self) -> None:
        ...
        print(
            f"🔥 User Custom: Epoch {getattr(self, 'current_epoch', 0)} starting (call #{self.custom_hook_calls['epoch_start']})"
        )
    ...
    def on_weavelet_train_epoch_end(self) -> None:
        ...
        print(
            f"✨ User Custom: Epoch {getattr(self, 'current_epoch', 0)} ended (call #{self.custom_hook_calls['epoch_end']})"
        )
    ...
    def on_weavelet_train_batch_start(self, batch, batch_idx):
        ...
        if batch_idx % 5 == 0:  # Only log every 5th batch
            print(
                f"⚡ User Custom: Starting batch {batch_idx} (total calls: {self.custom_hook_calls['batch_start']})"
            )
    ...
    def on_weavelet_train_batch_end(self, outputs, batch, batch_idx):
        ...
        if batch_idx % 5 == 0:  # Only log every 5th batch
            ...
            print(
                f"🎯 User Custom: Batch {batch_idx} loss: {loss:.4f} (total calls: {self.custom_hook_calls['batch_end']})"
            )
        return None
...
class LightningModuleWithExistingHooks(L.LightningModule):
    ...
    def __init__(self):
        ...
        self.model = TestModel()
        self.learning_rate = 0.001
        self.original_hook_calls = {"epoch_start": 0, "batch_start": 0}
    ...
    def on_train_epoch_start(self) -> None:
        ...
        print(
            f"🏃 Original Lightning Hook: Epoch start (call #{self.original_hook_calls['epoch_start']})"
        )
    ...
    def on_train_batch_start(self, batch, batch_idx):
        ...
        if batch_idx % 10 == 0:
            print(
                f"🏃 Original Lightning Hook: Batch {batch_idx} start (call #{self.original_hook_calls['batch_start']})"
            )
        return None
...
def test_wrapper_with_safe_hooks():
    ...
    try:
        ...
        if hasattr(weavelet_trainer, "weavelet") and hasattr(
            weavelet_trainer.weavelet, "_handler_registry"
        ):
        ...
        for batch_idx in range(3):
            ...
            weavelet_trainer._wrapped_on_train_batch_end(
                fake_outputs, fake_batch, batch_idx
            )
...
def test_wrapper_with_existing_hooks():
    ...
    try:
        ...
        print(
            f"✅ Created Lightning module with existing hooks: {type(lightning_module)}"
        )

        # Step 2: Wrap with weavelet functionality
        weavelet_trainer = WeaveletWrapper(
            lightning_module, replica_id="existing_hooks_trainer"
        )
        print(f"✅ Wrapped with WeaveletWrapper: {type(weavelet_trainer)}")
        ...
        print(
            "✅ Notice: Original Lightning hooks were preserved AND weavelet functionality added!"
        )
...
def test_convenience_function():
    ...
    try:
        ...
        lightning_module = SafeLightningModule()
        weavelet_trainer = make_weavelet(
            lightning_module, replica_id="convenience_trainer"
        )
...
def test_attribute_delegation():
    ...
    try:
        lightning_module = SafeLightningModule()
        weavelet_trainer = WeaveletWrapper(
            lightning_module, replica_id="delegation_trainer"
        )
...
def main():
    ...
    print(
        "This demonstrates the new wrapper approach for adding weavelet functionality"
    )
    print("=" * 80)
