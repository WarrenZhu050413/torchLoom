...
class WeaveletLightningModule(L.LightningModule):
    ...
    def _publish_training_status(self, batch_idx: int, result: Any) -> None:
        ...
        """
        try:
            # Extract loss from result with improved type handling
            loss_value = None
            if isinstance(result, dict) and "loss" in result:
                ...
                # Handle tensor objects (ensure it's not a dict first)
...
class AutoWeaveletMixin:
    ...
    registers them with the weavelet instance. Should be used with classes that have
    a 'weavelet' attribute.
    """

    # Type hint for classes that use this mixin
    weavelet: Weavelet

    def _register_weavelet_handlers(self):
        """Scan and register all methods decorated with @weavelet_handler."""
        if not hasattr(self, "weavelet") or self.weavelet is None:
            return
        ...
        for name in dir(self):
            ...
            try:
                ...
                if callable(method) and hasattr(method, "_weavelet_config_key"):
                    ...
                    print(
                        f"Auto-registered weavelet handler: {name} for '{config_key}'"
                    )
            except Exception:
