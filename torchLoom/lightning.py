...
class WeaveletWrapper(L.LightningModule):
    ...
    def __init__(self, lightning_module: L.LightningModule, replica_id: str, 
    ...
    def _register_weavelet_handlers(self):
        ...
        skip_attrs = {
            ...
            "trainer", "optimizers", "optimizer", "device", "global_rank", "local_rank",
            "fabric", "logger", "loggers", "checkpoint_callback", "callbacks",
            "datamodule", "current_epoch", "global_step", "automatic_optimization",
            
            # PyTorch module attributes that could cause issues
            "training", "named_parameters", "named_modules", "named_children",
            "parameters", "modules", "children", "state_dict", "load_state_dict",
    ...
    def _setup_hooks(self):
        ...
        self._original_on_train_batch_start = getattr(self._wrapped_module, 'on_train_batch_start', None)
        self._original_on_train_batch_end = getattr(self._wrapped_module, 'on_train_batch_end', None)
        self._original_on_train_epoch_start = getattr(self._wrapped_module, 'on_train_epoch_start', None)
        self._original_on_train_epoch_end = getattr(self._wrapped_module, 'on_train_epoch_end', None)
        self._original_on_train_end = getattr(self._wrapped_module, 'on_train_end', None)
        
        # Replace with our wrapped versions
        if hasattr(self._wrapped_module, '__dict__'):
            self._wrapped_module.on_train_batch_start = self._wrapped_on_train_batch_start
            self._wrapped_module.on_train_batch_end = self._wrapped_on_train_batch_end
            self._wrapped_module.on_train_epoch_start = self._wrapped_on_train_epoch_start
    ...
    def _wrapped_on_train_batch_start(self, batch, batch_idx):
        ...
        user_result = None
        if self._wrapped_module and hasattr(self._wrapped_module, 'on_weavelet_train_batch_start'):
            user_result = self._wrapped_module.on_weavelet_train_batch_start(batch, batch_idx)
    ...
    def _wrapped_on_train_batch_end(self, outputs, batch, batch_idx):
        ...
        # Call user's custom hook if they defined one
        user_result = None
        if self._wrapped_module and hasattr(self._wrapped_module, 'on_weavelet_train_batch_end'):
            user_result = self._wrapped_module.on_weavelet_train_batch_end(outputs, batch, batch_idx)
        
        # Call original method if it existed
        original_result = None
        if self._original_on_train_batch_end:
            original_result = self._original_on_train_batch_end(outputs, batch, batch_idx)
    ...
    def _wrapped_on_train_epoch_start(self):
        ...
        # Call user's custom hook if they defined one
        if self._wrapped_module and hasattr(self._wrapped_module, 'on_weavelet_train_epoch_start'):
    ...
    def _wrapped_on_train_epoch_end(self):
        ...
        # Call user's custom hook if they defined one
        if self._wrapped_module and hasattr(self._wrapped_module, 'on_weavelet_train_epoch_end'):
    ...
    def _publish_training_status(self, batch_idx: int, result: Any) -> None:
        ...
        try:
            ...
            elif (not isinstance(result, dict) and hasattr(result, "item") 
            ...
            status = {
                "epoch": getattr(self._wrapped_module, "current_epoch", 0) if self._wrapped_module else 0,
                "batch_idx": batch_idx,
            ...
            if self._wrapped_module and hasattr(self._wrapped_module, '_collect_additional_status'):
    ...
    def _publish_epoch_status(self) -> None:
        ...
        if hasattr(self, 'weavelet') and self.weavelet:
            status_data = {
                "type": "epoch_end",
                "epoch": getattr(self._wrapped_module, "current_epoch", 0) if self._wrapped_module else 0,
                "global_step": getattr(self._wrapped_module, "global_step", 0) if self._wrapped_module else 0,
            ...
            try:
                if (self._wrapped_module and 
                    hasattr(self._wrapped_module, "trainer") and 
                    self._wrapped_module.trainer is not None):
                    if hasattr(self._wrapped_module.trainer, 'logged_metrics'):
                        status_data["metrics"] = dict(self._wrapped_module.trainer.logged_metrics)
    ...
    def __getattr__(self, name):
        ...
        if '_wrapped_module' in self.__dict__ and self._wrapped_module and hasattr(self._wrapped_module, name):
    ...
    def __setattr__(self, name, value):
        ...
        if name in ('_wrapped_module', '_replica_id', 'weavelet', '_original_on_train_batch_start',
                   '_original_on_train_batch_end', '_original_on_train_epoch_start', 
                   '_original_on_train_epoch_end', '_original_on_train_end'):
            super().__setattr__(name, value)
        # Lightning framework attributes that should stay on wrapper
        elif name in ('_device_type', '_logger', '_fabric', '_precision', '_accelerator',
                     'prepare_data_per_node', 'allow_zero_length_dataloader_with_multiple_devices',
                     'automatic_optimization', '_forward_cache', '_running_stage', '_compile_config'):
            super().__setattr__(name, value)
        # Lightning properties that can't be set (read-only) - set on wrapper
        elif name in ('current_epoch', 'global_step', 'trainer', 'device', 'global_rank', 'local_rank'):
            try:
                super().__setattr__(name, value)
        ...
        elif hasattr(self, '_wrapped_module') and self._wrapped_module and hasattr(self._wrapped_module, name):
...
def make_weavelet(lightning_module: L.LightningModule, replica_id: str, 
                  torchLoom_addr: Optional[str] = None) -> WeaveletWrapper:
