...
from torchLoom.utils import get_device_uuid
...
from torchLoom.logging.logger import setup_logger
...
class Manager:
    ...
    def __init__(
        ...
        self._torchLoom_logger = _ManagerLogger(
            manager=self, 
            name="manager_torchLoom",
        ...
        # Start torchLoom
        self._subscriptions = {}
        self._torchLoom_addr = torchLoom_addr or os.environ.get("torchLoom_ADDR", torchLoomConstants.DEFAULT_ADDR)
        ...
        # Start the n
        self._loop.run_until_complete(self.torchLoom_start())
        ...
        self._torchLoom_logger.info("Asyncio event loop set up in background thread")

    async def torchLoom_start(self):   
        self._torchLoom_logger.info(f"In torchLoom_start")
        try:
            await self.torchLoom_connect()
            await self.subscribe_nc(torchLoomConstants.subjects.REPLICA_FAIL, self.message_handler)
            await self.publish_DRmapping_info()
            self._torchLoom_logger.info(f"torchLoom initialization completed successfully")
        except Exception as e:
            self._torchLoom_logger.exception(f"Error during torchLoom_start: {e}")

    async def torchLoom_connect(self):
        try:
            self._nc: Client = await nats.connect(self._torchLoom_addr)
            self._js: JetStreamContext = self._nc.jetstream()
            self._torchLoom_logger.info("Connected to NATS server")
        except Exception as e:
            self._torchLoom_logger.error(f"Failed to connect to NATS server: {e}")
    ...
    async def message_handler(self, msg: Msg):
        try:
            ...
            self._torchLoom_logger.info(f"Received message on {msg.subject}: {msg.data}")

            if env.HasField("replica_fail"):
                replica_id = env.replica_fail.replica_id
                self._torchLoom_logger.info(f"Received replica fail event for replica: {replica_id}")
                # Check if this is our replica
                if replica_id == self._replica_id:
                    self._torchLoom_logger.info(f"This replica ({self._replica_id}) has been marked as failed, initiating recovery")
                    # Report an error to trigger recovery
                    self._torchLoom_logger.info(f"MOCK IMPLEMENTATION: Reporting error to trigger recovery")
            else:
                self._torchLoom_logger.warning(f"Received unsupported message: {env}")
        except Exception as e:
            self._torchLoom_logger.exception(f"Error handling message: {e}")

    async def subscribe_nc(self, subject: str, message_handler: Callable[[Msg], Awaitable[None]]) -> None:
        assert self._nc is not None # This should always be true because we connect to NATS in torchLoom_start in Manager constructor

        sub = await self._nc.subscribe(subject)
        self._torchLoom_logger.info(f"Subscribed to {subject}")
        
        async def listen_to_nc_subscription():
            self._torchLoom_logger.info(f"Started listening on {subject}")
            try:
                self._torchLoom_logger.info(f"self._nc_timeout: {self._nc_timeout}")
                self._torchLoom_logger.info(f"self._stop_nats.is_set(): {self._stop_nats.is_set()}")
                while self._stop_nats.is_set() is False:
                    try:
                        self._torchLoom_logger.info(f"Waiting for message on {subject}")
                        msg = await sub.next_msg(timeout=self._nc_timeout)
                        self._torchLoom_logger.info(f"Received message on {subject}: {msg.data}")
                        await message_handler(msg)
                    except asyncio.TimeoutError:
                        continue
                    except Exception as e:
                        self._torchLoom_logger.exception(f"Error in subscription loop for {subject}: {str(e)}")
                        await asyncio.sleep(self._exception_sleep)
            except Exception as e:
                self._torchLoom_logger.exception(f"Error in listener task: {str(e)}")
    ...
    async def publish_DRmapping_info(self):
        ...
        await self._js.publish(torchLoomConstants.controller_stream.subjects.DR_SUBJECT, DRenvelope.SerializeToString())
    # End --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def shutdown(self, wait: bool = True) -> None:
        ...
        self._torchLoom_logger.info("NATS connection closed")
        
        # Stop the event loop
        self._loop.call_soon_threadsafe(self._loop.stop)
        self._torchLoom_logger.info("Event loop stopped")
        
        # Shut down the loop thread
        self._loop_thread.shutdown(wait=wait)
        self._torchLoom_logger.info("Background loop thread shutdown")
    ...
    def _run_event_loop_in_background(self):
        ...
        try:
            if not self._loop.is_running():
                self._torchLoom_logger.info("Starting asyncio event loop in background")
                self._loop.run_forever()
            else:
                self._torchLoom_logger.info("Event loop is already running")
        except Exception as e:
            self._torchLoom_logger.exception(f"Error in background event loop: {e}")
