...
from torchLoom.proto.torchLoom_pb2 import UIStatusUpdate, deviceStatus, TrainingStatus
...
class WeaverUIState:
    ...
    communication_status: str = "stable"  # Overall communication status with other components
    ...
    def update_training_progress_from_proto(self, ts_proto: TrainingStatus):
        ...
        if found_idx != -1:
            ...
            logger.debug(f"Updated existing replica training status in UI state: {ts_proto.replica_id}")
        else:
            ...
            logger.debug(f"Added new replica training status to UI state: {ts_proto.replica_id}")
...
if __name__ == '__main__':
    ...
    logger.info("Starting WeaverUIState example...")
    ui_state_manager = WeaverUIState()
    ...
    for dev in current_snapshot.devices:
        logger.info(f"Device: {dev.device_id}, Status: {dev.status}, Util: {dev.utilization}%")
    for ts in current_snapshot.training_status:
        logger.info(f"Replica: {ts.replica_id}, Step: {ts.current_step}, Metrics: {ts.metrics}")
    ...
    logger.info("WeaverUIState example finished.") 