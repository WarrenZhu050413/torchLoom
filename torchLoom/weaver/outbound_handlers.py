...
class UIUpdateHandler(OutboundHandler):
    ...
    async def publish_ui_update(self) -> None:
        ...
        try:
            ...
            logger.debug(f"[WEAVER->UI] Published UI update: global_step={ui_update.global_step}")
...
class WeaveletCommandHandler(OutboundHandler):
    ...
    async def publish_replica_fail_event(self, replica_id: str) -> None:
        ...
        try:
            if not self.nats_client:
                logger.warning("[WEAVER->WEAVELET] Cannot publish replica fail event - no NATS client")
                return
    ...
    async def publish_weaver_command(self, command_type: str, target_replica_id: str, params: dict = None) -> None:
        ...
        try:
            if not self.nats_client:
                logger.warning("[WEAVER->WEAVELET] Cannot publish weaver command - no NATS client")
                return
    ...
    async def publish_config_update(self, config_params: dict) -> None:
        ...
        try:
            if not self.nats_client:
                logger.warning("[WEAVER->WEAVELET] Cannot publish config update - no NATS client")
                return
...
class DemoDataSimulator:
    ...
    def initialize_demo_data(self):
        ...
        logger.info(f"[DEMO] Initialized demo data: {gpu_counter} GPUs across {len(self.demo_replicas)} replicas")
