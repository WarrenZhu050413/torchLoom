        ...
        async def run_test():
            async with NatsTestServer() as nats_url:
                ...
                try:
                    ...
                    assert init_time < 2.0, f"Weaver initialization took too long: {init_time:.2f}s"
                    ...
                    assert shutdown_time < 1.0, f"Weaver shutdown took too long: {shutdown_time:.2f}s"
            ...
            async with NatsTestServer() as nats_url:
                ...
                try:
                    ...
                    async with TorchLoomClient(nats_url) as cli_client:
                        ...
                        await cli_client.register_device("gpu1", "replica3")  # Multi-replica assignment
                    ...
                    assert weaver.get_replicas_for_device("gpu1") == {"replica1", "replica3"}
                    ...
                    assert workflow_time < 5.0, f"Workflow took too long: {workflow_time:.2f}s"
                ...
                async with NatsTestServer(port=4225) as nats_url2:
                    ...
                    try:
                        ...
                        async with TorchLoomClient(nats_url1) as cli1:
                            await cli1.register_device("weaver1_device", "weaver1_replica")
                        
                        async with TorchLoomClient(nats_url2) as cli2:
                            await cli2.register_device("weaver2_device", "weaver2_replica")
        ...
        async def run_test():
            async with NatsTestServer() as nats_url:
                ...
                try:
                    ...
                    for i in range(3):
                        async with TorchLoomClient(nats_url) as cli_client:
                            await cli_client.register_device(f"device_{i}", f"replica_{i}")
                    ...
                    async with TorchLoomClient(nats_url) as final_client:
                        await final_client.register_device("final_device", "final_replica")
        ...
        async def run_test():
            async with NatsTestServer() as nats_url:
                ...
                try:
                    ...
                    assert processed_count[0] == message_count, f"Only processed {processed_count[0]}/{message_count} messages"
                    ...
                    print(f"Processed {message_count} messages in {processing_time:.2f}s ({message_count/processing_time:.1f} msg/s)")
